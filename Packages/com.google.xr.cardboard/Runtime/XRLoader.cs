...
namespace Google.XR.Cardboard
{
    ...
    public class XRLoader : XRLoaderHelper
    {
        ...
        {
            // Patched 2026-07-25: upstream Cardboard passes Screen.safeArea here to avoid
            // rendering behind a camera notch/cutout. That makes sense for normal phone use,
            // but this device is permanently sealed inside a light-blocking VR headset tray -
            // nothing is using the camera, and the safe-area exclusion (Pixel 10's cutout inset,
            // ~173px in portrait) becomes a large one-sided asymmetric black margin once rotated
            // into landscape VR, since it only affects one edge. Force full-screen bounds instead.
            renderingArea = new Rect(0, 0, Screen.width, Screen.height);
            ...
            // Patched 2026-07-25: the alignment-line widget (a thin vertical divider between the
            // two eye views) renders as a large jagged yellow artifact on this device once the
            // safe-area patch above changed the rendering-area rect it's computed relative to -
            // its rect ends up wrong-sized. Not essential (purely a visual eye-alignment aid), so
            // drop it rather than chase the exact native-side math.
